...
- **Воспроизводимость**: идентификаторы детерминированы (содержательные, не случайные), отчёт печатается SHA-256 (`integrity.reportDigest` + `evidenceDigest`); повторный запуск на тех же данных даёт те же id и тот же риск (проверяется в `npm run verify`).
