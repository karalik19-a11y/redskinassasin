  ...
  return (
    <div className="w-full flex justify-center items-center py-1 select-none z-50">
      <motion.div
        layout
        onClick={handleTap}
        className={`bg-panel text-white cursor-pointer shadow-[0_4px_24px_rgba(0,0,0,0.8)] border border-hair overflow-hidden ${
          isExpanded
            ? 'w-[90%] rounded-[20px] p-4'
            : isScanning
          ...
          {isExpanded ? (
            ...
            >
              <div className="flex items-center justify-between border-b border-hair pb-2">
                <div className="flex items-center space-x-2">
                  <div className="w-2.5 h-2.5 rounded-full bg-clay animate-pulse-totem" />
                  <span className="font-bold tracking-wider text-clay text-[11px]">
                    TOMAHAWK CYBER-SHAMAN HUD
                  </span>
                </div>
                <span className="text-[10px] text-gold font-mono">v4.8 PRO</span>
              </div>

              <div className="grid grid-cols-2 gap-2 text-[11px]">
                <div className="bg-white/5 rounded-xl p-2 border border-hair">
                ...
                <div className="bg-white/5 rounded-xl p-2 border border-hair">
                  ...
                  <div className="font-semibold text-sage flex items-center space-x-1">
                    <ShieldAlert className="w-3 h-3 text-clay" />
              ...
              <div className="flex items-center justify-between pt-1 text-[10px] text-white/50">
                ...
                <span className="text-clay/80 font-mono">Шифрование ГОСТ 256</span>
              </div>
            </motion.div>
          ) : isScanning ? (
            <motion.div
            ...
            >
              <div className="flex items-center space-x-2">
                <div className="relative flex items-center justify-center">
                  ...
                  <span className="absolute w-2 h-2 rounded-full bg-clay" />
                </div>
              ...
              <div className="flex items-center space-x-2">
                <div className="w-12 h-1.5 bg-white/20 rounded-full overflow-hidden">
                  <div
                    className="h-full bg-gradient-to-r from-[#c2664f] to-[#a8834c] transition-all duration-150"
                    style={{ width: `${scanProgress}%` }}
                  />
                </div>
                <span className="text-[10px] font-mono text-gold">{scanProgress}%</span>
              </div>
            </motion.div>
          ) : (
            <motion.div
            ...
            >
              ...
              <div className="flex items-center space-x-1.5">
                <span className="w-2 h-2 rounded-full bg-clay/80 shadow-[0_10px_30px_rgba(0,0,0,.28)]" />
                <Sparkles className="w-3 h-3 text-gold" />
              </div>

              {/* Camera cutout simulator */}
              <div className="w-3 h-3 rounded-full bg-panel border border-hair" />

              {/* Right Island Activity */}
              <div className="flex items-center space-x-1">
                <Activity className="w-3 h-3 text-clay animate-pulse" />
              </div>
            </motion.div>
          )}
        </AnimatePresence>
