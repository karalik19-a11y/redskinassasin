...
export const DreamcatcherGraph: React.FC<DreamcatcherGraphProps> = ({ dossier }) => {
  ...
  return (
    <div className="space-y-3 pb-20 select-none">
      ...
      <div className="relative w-full aspect-square max-h-[460px] rounded-[20px] bg-gradient-to-b from-neutral-950 via-[#0a0812] to-neutral-950 border border-hair overflow-hidden flex items-center justify-center shadow-2xl">
        ...
        >
          {/* Circular Rings */}
          <div className="absolute w-[170px] h-[170px] rounded-full border border-hair border-dashed" />
          <div className="absolute w-[290px] h-[290px] rounded-full border border-hair" />
          <div className="absolute w-[390px] h-[390px] rounded-full border border-hair border-dotted" />
              ...
              >
                <div
                  ...
                  className={`rounded-full flex items-center justify-center border-2 shadow-lg transition-all ${
                    isCenter
                      ? 'w-14 h-14 text-white shadow-[0_10px_30px_rgba(0,0,0,.28)]'
                      : 'w-8 h-8 text-ink shadow-md hover:shadow-[0_10px_30px_rgba(0,0,0,.28)]'
                  }`}
                >
                  {isCenter ? (
                    <Shield className="w-6 h-6 animate-pulse text-gold" />
                  ) : (
                ...
                <span
                  className={`mt-1 px-1.5 py-0.2 rounded-md text-[9px] font-mono whitespace-nowrap bg-panel border border-hair ${
                    isSelected ? 'text-gold font-bold border-hair' : 'text-ink'
                  }`}
                >
                  {node.label}
                </span>
        ...
        {selectedNode && (
          <div className="absolute bottom-2 left-2 right-2 p-2.5 bg-panel backdrop-blur-xl rounded-[16px] border border-hair text-xs text-white flex items-center justify-between z-40 shadow-2xl">
            <div className="min-w-0 flex-1 pr-2">
              <div className="text-[10px] font-mono text-gold font-bold uppercase flex items-center space-x-1">
      ...
      {/* Legend */}
      <div className="ios-glass p-2.5 rounded-[16px] border border-hair flex flex-wrap items-center justify-around gap-2 text-[10px] text-muted font-mono">
        <span className="flex items-center space-x-1"><span className="w-2 h-2 rounded-full bg-clay" /><span>Цель</span></span>
        <span className="flex items-center space-x-1"><span className="w-2 h-2 rounded-full bg-gold-soft" /><span>Родственники</span></span>
        <span className="flex items-center space-x-1"><span className="w-2 h-2 rounded-full bg-sage-soft" /><span>Банки</span></span>
        <span className="flex items-center space-x-1"><span className="w-2 h-2 rounded-full bg-violet-soft" /><span>Авто/ТС</span></span>
